...
Loop:
	for {
		select {
		case <-ticker.C:
			idents, err := l.Service.RPCGetServices(ctx, "lightOrchastrator")
			if err != nil {
				system.Logf("Error locating lightOrchastrator: %v", err.Error())
				continue
			}
			if len(idents) > 1 {
				system.Logf("Found multiple lightOrchastrator, %v", idents)
				continue
			}
			if len(idents) < 1 {
				continue
